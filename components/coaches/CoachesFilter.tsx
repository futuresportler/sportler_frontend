...
export default function CoachesFilter({ filterOptions, onFilterChange }: CoachesFilterProps) {
  ...
  const handleTrainedProfessionalsChange = () => {
    onFilterChange({
      trainedProfessionals: !filterOptions.trainedProfessionals,
    })
  }
    ...
    <div className="bg-white text-gray-800 rounded-lg p-6 sticky top-24 shadow-md border border-gray-100">
      ...
      <div className="mb-6">
        ...
        {expandedSections.sessionType && (
          <div className="space-y-3">
            {["One-on-One", "Group Training", "Online Coaching", "In-Person Training"].map((type) => (
      ...
      <div className="mb-6">
        ...
        {expandedSections.availabilityTimeSlots && (
          <div className="space-y-3">
            {["Morning", "Afternoon", "Evening", "Custom Time Selection"].map((slot) => (
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <h3 className="font-medium">Trained Professional Athletes Before</h3>
          <div className="relative inline-block w-10 align-middle select-none">
            <input
              type="checkbox"
              id="trained-professionals"
              checked={filterOptions.trainedProfessionals}
              onChange={handleTrainedProfessionalsChange}
              className="sr-only"
            />
            <label
              htmlFor="trained-professionals"
              className={`block h-6 overflow-hidden rounded-full cursor-pointer ${
                filterOptions.trainedProfessionals ? "bg-emerald-400" : "bg-gray-300"
              }`}
            >
              <span
                className={`block h-6 w-6 rounded-full bg-white transform transition-transform duration-200 ease-in-out ${
                  filterOptions.trainedProfessionals ? "translate-x-4" : "translate-x-0"
                }`}
              ></span>
            </label>
          </div>
